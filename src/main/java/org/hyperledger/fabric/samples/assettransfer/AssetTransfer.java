...
public final class AssetTransfer implements ContractInterface {
    ...
    /**
     * Duplicate asset from one person to another.
     *
     * @param ctx the transaction context
    ...
    public Asset DuplicateAsset(final Context ctx, final String assetID, final String newOwner) {
        ...
        if (asset.getOwner().equals(newOwner)) {
            String errorMessage = String.format("Asset %s cannot duplicate to same owner", assetID);
            System.out.println(errorMessage);
            throw new ChaincodeException(errorMessage, AssetTransferErrors.SAME_USER.toString());
        }

        int iterator = 1;
        String candidateAssetName = "asset";
        while (true) {
            String nominateAssetName = candidateAssetName + String.valueOf(iterator);
            String assetJSONGenerated = stub.getStringState(nominateAssetName);
            if (assetJSONGenerated == null || assetJSONGenerated.isEmpty()) {
                candidateAssetName = nominateAssetName;
                break;
            }
            iterator++;
        }

        Asset newAsset = new Asset(candidateAssetName, asset.getColor(), asset.getSize(), newOwner, asset.getAppraisedValue());
        //Use a Genson to conver the Asset into string, sort it alphabetically and serialize it into a json string
        String sortedJson = genson.serialize(newAsset);
        stub.putStringState(candidateAssetName, sortedJson);
